...
def run_bindetect(args):
    ...
    args.cond_names = (
        ...
        if args.cond_names is None
        else args.cond_names
    )
    ...
    outfiles = [
        os.path.abspath(
            os.path.join(
                args.outdir, "*", "beds", "*_{0}_{1}.bed".format(condition, state)
            )
        )
    ...
    if len(args.cond_names) != len(args.signals):
        logger.error(
            "The given number of given '--cond-names' ({0}) differ from the given input '--signals' ({1}). Please enter one condition name per signal.".format(
                len(args.cond_names), len(args.signals)
            )
        )
        sys.exit(1)
    ...
    if len(args.cond_names) != len(set(args.cond_names)):
        logger.error(
            "The condition names are not unique ({0}). Please use --cond-names to set a unique set of condition names.".format(
                args.cond_names
            )
        )
        sys.exit(1)
    ...
    if args.time_series:
        ...
        comparisons = list(
            itertools.combinations(args.cond_names, 2)
        )  # all-against-all
        args.comparisons = comparisons
    ...
    plt.text(
        ...
        0.6,
        "\n".join(
            ["Page {0}) {1}".format(i + 2, titles[i]) for i in range(len(titles))]
        )
        + "\n\n",
        va="top",
    )
    ...
    for i, peak in enumerate(peaks):
        if len(peak) != n_cols:
            logger.error(
                "The lines in --peaks have a varying number of columns. Line 1 has {0} columns, but line {1} has {2} columns! Please adjust the format of this file to run TOBIAS BINDetect.".format(
                    n_cols, i + 1, len(peak)
                )
            )
            sys.exit(1)
    ...
    if args.peak_header != None:
        ...
        if len(args.peak_header_list) != peak_columns:
            logger.error(
                "Length of --peak_header ({0}) does not fit number of columns in --peaks ({1}).".format(
                    len(args.peak_header_list), peak_columns
                )
            )
            sys.exit(1)
    else:
    ...
    peaks = peaks.apply_method(
        OneRegion.check_boundary, fasta_boundaries, "cut"
    )  # will exit if peaks are outside borders
    ...
    for signal in args.signals:
        ...
        peaks = peaks.apply_method(OneRegion.check_boundary, pybw_header, "cut")
    ...
    args.gc = gc_content
    bg = np.array(
        [(1 - args.gc) / 2.0, args.gc / 2.0, args.gc / 2.0, (1 - args.gc) / 2.0]
    )
    ...
    for f in args.motifs:
        try:
            ...
            logger.error(
                "Error reading motifs from '{0}'. Error message was: {1}".format(f, e)
            )
            sys.exit(1)
    ...
    outlist = pool.starmap(
        OneMotif.get_threshold, itertools.product(motif_list, [args.motif_pvalue])
    )
    ...
    logo_filenames = {
        motif.prefix: os.path.join(args.outdir, motif.prefix, motif.prefix + ".png")
        for motif in motif_list
    }
    ...
    for TF_names_sub in TF_names_chunks:
        ...
        files = [
            os.path.join(args.outdir, TF, "beds", TF + ".tmp") for TF in TF_names_sub
        ]
    ...
    while finished == 0:
        logger.debug(
            "Writer task return status: {0}".format(
    ...
    if n_bg_values < 1000:
        err_str = (
            "Number of background values collected from peaks is low (={0}) ".format(
                n_bg_values
            )
        )
    ...
    args.norm_objects = {}
    if (
        args.norm_off == True or len(args.cond_names) == 1
    ):  # if norm_off or length of cond is 1 - create constant normalization
        for bigwig in args.cond_names:
        ...
        if args.debug:
            args.norm_objects = quantile_normalization(
                list_of_vals, args.cond_names, pdfpages=debug_pdf, logger=logger
            )
        else:
            args.norm_objects = quantile_normalization(
                list_of_vals, args.cond_names, logger=logger
            )
        ...
        for bigwig in args.cond_names:
            ...
            # Check for nan
            logger.debug(
                "Background nans ({0}): {1}".format(bigwig, sum(np.isnan(original)))
            )
    ...
    bg_values = bg_values[
        np.logical_not(np.isclose(bg_values, 0.0))
    ]  # only non-zero counts
    logger.debug(
        "Size of background array after filtering > 0: {0}".format(bg_values.size)
    )
    if len(bg_values) == 0:
        logger.error(
    ...
    if args.debug:
        ...
        for i in range(len(means)):
            ...
            log_params = scipy.stats.lognorm.fit(
                bg_values, f0=stds[i], fscale=np.exp(means[i])
            )
    ...
    log_params = scipy.stats.lognorm.fit(
        bg_values, f0=stds[chosen_i], fscale=np.exp(means[chosen_i])
    )

    # Mode of distribution
    mode = scipy.optimize.fmin(
        lambda x: -scipy.stats.lognorm.pdf(x, *log_params), 0, disp=False
    )[0]
    ...
    mirrored_x = np.concatenate(
        [leftside_x, np.max(leftside_x) + leftside_x_scale]
    ).flatten()
        ...
        for bigwig1, bigwig2 in comparisons:  # cond1, cond2
            ...
            if args.debug:
                ...
                plt.hist(
                    ...
                    density=True,
                    bins="auto",
                    label="Background log2fc ({0} / {1})".format(bigwig1, bigwig2),
                )
    ...
    info_columns.extend(
        [
            ...
            for (cond, metric) in itertools.product(
                args.cond_names, ["threshold", "bound"]
            )
        ]
    )
    info_columns.extend(
        [
            ...
            for (comparison, metric) in itertools.product(
                comparisons, ["change", "pvalue"]
            )
        ]
    )
    ...
    info_table = pd.DataFrame(
        np.zeros((rows, cols)), columns=info_columns, index=motif_names
    )
    ...
    for cluster in clustering.clusters:
        for name in convert:
            clustering.clusters[cluster]["cluster_name"] = clustering.clusters[cluster][
                "cluster_name"
    ...
    info_table_clustered = info_table.groupby("cluster").mean(
        numeric_only=True
    )  # mean of each column
    ...
    for cond1, cond2 in comparisons:
        ...
        change_min, change_max = np.percentile(
            changes, [5, 95]
        )  # 5% smallest and largest changes
    ...
    if no_conditions > 1:
        ...
        for cond1, cond2 in comparisons:
            ...
            for motif in motif_list:
                name = motif.prefix
                motif.change = float(
                    info_table.at[name, base + "_change"]
                )  # change for this comparison
                motif.pvalue = float(
                    info_table.at[name, base + "_pvalue"]
                )  # pvalue for this comparison
                motif.logpvalue = (
                    -np.log10(motif.pvalue) if motif.pvalue > 0 else -np.log10(1e-308)
                )
    ...
    if args.debug and len(args.signals) > 1:
        ...
        cm.ax_heatmap.set_ylabel(
            "Transcription factor motifs", fontsize=15, rotation=270
        )
